...
const props = withDefaults(defineProps<{ delta: number; percent?: boolean; kpi: string }>(), {
  percent: true,
});
...
const text = computed(() =>
  props.percent ? (Math.abs(props.delta) * 100).toFixed(1) + '%' : Math.abs(props.delta).toFixed(2)
);
