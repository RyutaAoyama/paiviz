<template>
  <div class="relative w-full">
    ...
    />
    <div
      v-if="open && (hasAny || q.length >= 2)"
      class="absolute left-0 right-0 mt-1 rounded-xl border border-[#242A33] bg-[#0F1115] shadow-2xl"
    >
      <!-- お気に入り -->
      <div v-if="favList.length" class="px-3 pt-3 text-xs text-muted">
        お気に入り
      </div>
      <ul v-if="favList.length" class="max-h-48 overflow-auto py-1">
        <li v-for="(n, i) in favList" :key="'f:' + n">
          <button
            class="item"
            :class="cls(i, 'fav')"
            @mousedown.prevent="go(n)"
            @mouseenter="setActive('fav', i)"
          >
      ...
      <!-- 最近 -->
      <div v-if="recentList.length" class="px-3 pt-3 text-xs text-muted">
        最近
      </div>
      <ul v-if="recentList.length" class="maxハ-48 overflow-auto py-1">
      ...
      <ul class="max-h-64 overflow-auto py-1">
        <li v-if="q.length < 2" class="px-3 py-2 text-xs text-muted">
          2文字以上入力すると候補が表示されます
        </li>
        <li v-else-if="loading" class="px-3 py-2 text-xs text-muted">
          検索中…
        </li>
        <li
        ...
        <li v-for="(s, i) in suggestions" :key="'s:' + s.name">
          <button
            class="item"
            :class="cls(i, 'suggest')"
            @mousedown.prevent="go(s.name)"
            @mouseenter="setActive('suggest', i)"
          >
...
const favList = computed(() => filterLocal(favs.value, q.value));
const { recent, addRecent } = useRecent();
const recentList = computed(() => filterLocal(recent.value, q.value));

const hasAny = computed(
  () =>
    favList.value.length || recentList.value.length || suggestions.value.length
);

let activeIndex = ref(0);
let activeSection = ref<Section>("suggest");
const activeOrCurrent = computed(() => {
...
function filterLocal(list: string[], q: string) {
  ...
  return list.filter((n) => n.toLowerCase().includes(qq)).slice(0, 5);
}

function hl(name: string) {
  const qq = q.value;
  if (!qq) return escapeHtml(name);
  const idx = name.toLowerCase().indexOf(qq.toLowerCase());
