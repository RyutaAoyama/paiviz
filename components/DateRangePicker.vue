<template>
  <div class="flex flex-wrap items-center gap-2">
    ...
    <div v-if="mode === 'custom'" class="flex items-center gap-2">
      ...
      <input v-model="to" type="date" class="rounded-xl bg-surface px-2 py-1 ring-1 ring-border" />
      <button
        class="rounded-lg border border-border px-2 py-1 text-sm hover:text-text"
        @click="setThisMonth"
      >
        今月に戻す
      </button>
...
const emit = defineEmits<{
  (e: 'update', payload: { mode: string; from: string; to: string }): void;
}>();
function fmt(d: Date) {
...
watch(
  ...
  () => {
    ...
    if (mode.value === 'last30d') {
      const r = lastDays(30);
      from.value = r.from;
      to.value = r.to;
    }
    if (mode.value === 'last90d') {
      const r = lastDays(90);
      from.value = r.from;
      to.value = r.to;
    }
