<template>
  <teleport to="body">
    <div v-if="open" class="fixed inset-0 z-50">
      ...
      <div
        class="absolute inset-x-0 bottom-0 rounded-t-2xl border border-[#242A33] bg-[#0F1115] p-4 shadow-2xl md:inset-y-0 md:left-auto md:right-0 md:w-[420px] md:rounded-l-2xl md:rounded-tr-none"
      >
        <div class="mb-3 flex items-center justify-between">
          <h3 class="text-base font-semibold">詳細フィルタ</h3>
          <button class="rounded-md border border-[#242A33] px-2 py-1 text-sm" @click="close">
            閉じる
          </button>
        </div>
