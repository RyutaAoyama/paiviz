<template>
  <div class="relative">
    <input
      ...
      @blur="onBlur"
    />
    <div v-if="open && suggestions.length" class="absolute z-50 mt-1 w-full rounded-xl border border-border bg-surface p-2 text-sm shadow-card">
...
const props = withDefaults(defineProps<{ modelValue: string; placeholder?: string }>(), { modelValue: '', placeholder: '' })
...
function onBlur(){ window.setTimeout(() => open.value = false, 120) }
function pick(n: string) { emit('update:modelValue', n); open.value = false }
</script>
