  ...
  <section class="space-y-4">
    <div class="flex items-center justify-between">
      ...
      <div class="hidden md:flex items-center gap-2 text-sm text-muted">
        ...
        <button class="rounded-lg border border-border px-2 py-1 hover:text-text" @click="toggleDense">
          密度: {{ dense ? '凝縮' : '快適' }}
        </button>
      </div>
    </div>
    ...
    <SkeletonTable v-if="loading" :rows="Math.ceil((dense ? 480 : 560)/rowHeight)" />
    ...
    <div
      ...
      @scroll="onScroll"
    >
      <div class="sticky top-0 grid grid-cols-[72px_1fr_90px_160px_90px] gap-3 border-b border-border bg-surface/80 px-3 py-2 text-xs text-muted backdrop-blur">
        <button class="text-left" @click="setSort('rank')" :aria-sort="aria('rank')"># <SortIcon :dir="icon('rank')" /></button>
        <button class="text-left" @click="setSort('name')" :aria-sort="aria('name')">名前 <SortIcon :dir="icon('name')" /></button>
        <button class="text-left" @click="setSort('rate')" :aria-sort="aria('rate')">Rate <SortIcon :dir="icon('rate')" /></button>
        <div>直近</div>
        <button class="text-left" @click="setSort('games')" :aria-sort="aria('games')">対局数 <SortIcon :dir="icon('games')" /></button>
      </div>
      ...
      <div v-else :style="{ height: totalHeight + 'px', position: 'relative' }" role="table" aria-label="ランキング">
        <div
          ...
          :style="{ position: 'absolute', top: (startIndex + i) * rowHeight + 'px', left: 0, right: 0, height: rowHeight + 'px', padding: '0 12px' }"
    ...
    <div class="flex items-center justify-between">
      <div class="text-xs text-muted">
        ヒント: フィルタはURLに保存されます。スマホでは下部ナビから主要ページに素早く移動できます。
      </div>
      <div class="md:hidden flex items-center gap-2">
        <button class="rounded-lg border border-border px-3 py-1.5 text-sm hover:text-text" @click="shareCurrent">
          共有リンク作成
        </button>
      </div>
    </div>
...
const asSortKey = (v:any): SortKey => SORTS.includes(v)  ? v as SortKey : 'rate'
...
const chipModel = computed<{ mode:Mode; tableType:Table; rule:Rule; sortKey:SortKey; favOnly:boolean }>({
  get: () => ({ mode: mode.value, tableType: tableType.value, rule: rule.value, sortKey: sort.key, favOnly: favOnly.value }),
  set: (v) => { mode.value = v.mode; tableType.value = v.tableType; rule.value = v.rule; sort.key = v.sortKey; favOnly.value = v.favOnly }
})

watch([mode, from, to, tableType, rule, dense, favOnly, () => sort.key, () => sort.dir], () => {
  router.replace({ query: {
    ...route.query,
    mode: mode.value, from: from.value, to: to.value,
    tableType: tableType.value, rule: rule.value,
    dense: String(dense.value), favOnly: String(favOnly.value),
    sortKey: sort.key, sortDir: sort.dir
  }})
})

function onRange(p: { mode: string; from: string; to: string }) {
  mode.value = asMode(p.mode); from.value = p.from; to.value = p.to
}
...
function aria(k: SortKey){ return sort.key === k ? (sort.dir === 'asc' ? 'ascending' : 'descending') : 'none' }
function icon(k: SortKey){ return sort.key === k ? sort.dir : 'none' }
...
const filtered = computed(() => {
  ...
  if (from.value && to.value) {
    ...
    const days = mode.value==='this' ? 31 : mode.value==='prev' ? 31 : mode.value==='last90d' ? 90 : 30
    const fromD = new Date(Date.now() - days*86400000)
    return list.filter(r => r.playedAt >= fromD)
  }
})

const favFiltered = computed(() => favOnly.value ? filtered.value.filter(r => isFav(r.name)) : filtered.value)

const sorted = computed(() => {
  const base = [...favFiltered.value]
  const dir = sort.dir === 'asc' ? 1 : -1
  if (sort.key === 'rank') return dir === 1 ? base : base.reverse()
  if (sort.key === 'name')  return base.sort((a, b) => a.name.localeCompare(b.name) * dir)
  if (sort.key === 'games') return base.sort((a, b) => (a.games - b.games) * dir)
...
const visibleRows = computed(() => sorted.value.slice(startIndex.value, startIndex.value + visibleCount.value))
const totalHeight = computed(() => sorted.value.length * rowHeight.value)

function toggleDense(){ dense.value = !dense.value }
function isSelected(i: number) { return selected.value === i }

function onKey(e: KeyboardEvent) {
  if (e.key === 'ArrowDown') { e.preventDefault(); selected.value = Math.min(sorted.value.length - 1, selected.value + 1); ensureVisible() }
  else if (e.key === 'ArrowUp') { e.preventDefault(); selected.value = Math.max(0, selected.value - 1); ensureVisible() }
  else if (e.key === 'Enter') { e.preventDefault(); goDetail(selected.value) }
}
function ensureVisible() {
  ...
  if (bottom > vp.scrollTop + vp.clientHeight) vp.scrollTop = bottom - vp.clientHeight
}
...
async function shareCurrent() {
  try {
    const short = await createShareLink('rankings', {
      mode: mode.value, from: from.value, to: to.value,
      tableType: tableType.value, rule: rule.value,
      dense: dense.value, favOnly: favOnly.value,
      sortKey: sort.key, sortDir: sort.dir
    })
...
function onExportCsv() {
  ...
  const rows = sorted.value.map((r, idx) => ({ rank: idx + 1, name: r.name, rate: r.rate, games: r.games })).slice(0, 500)
  downloadCsv('paiviz_rankings.csv', rows, header, { rank:'#', name:'名前', rate:'Rate', games:'対局数' })
