...
export interface ShortenResponse { id: string; short: string }
export interface SnapshotPayload { v?: number; kind?: ShareKind | string; data?: Record<string, any> | null; createdAt?: number }
export interface SuggestItem { name: string; source: string }
export interface SuggestResponse { suggestions: SuggestItem[] }
...
export async function apiFetch<T>(path: string, opts: ApiOptions = {}): Promise<T> {
  const {
    method='GET', body, timeoutMs=8000, headers={}, retries=1
  } = opts
...
// ---- 既存API（互換） ----
export async function createShareLink(kind: ShareKind, data: Record<string, any>) {
  const res = await apiFetch<ShortenResponse>('/api/shorten', { method:'POST', body:{ kind, data }, retries:2 })
  return res.short
}
export function getSnapshot(id: string) {
  return apiFetch<SnapshotPayload>(`/api/snapshot?id=${encodeURIComponent(id)}`, { retries:1 })
}
export async function getSuggest(q: string, limit=10): Promise<SuggestItem[]> {
  if (!q || q.trim().length < 2) return []
  const res = await apiFetch<SuggestResponse>(`/api/suggest?q=${encodeURIComponent(q)}&limit=${limit}`, { retries:1 })
